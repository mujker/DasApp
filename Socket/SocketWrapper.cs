...
namespace DasApp.Socket
{
    public class SocketWrapper : IDisposable
    {
        ...
        /// 密钥长度为8
        /// </summary>
        private string _cryptKey = "hxsoft++";
            ...
            lock (locker)
            {
                try
                {
                    if (isEncrypt)
                    {
                        ...
                        sendData = DataPacketCodec.Encode(sendData, CryptKey);
                        sendData += "#";
                    }
                    try
                    {
                    ...
                    if (isEncrypt)
                    {
                        ...
                        strReceive = DataPacketCodec.Decode(strReceive, CryptKey);
                    }
                    return strReceive;
                }
                catch
