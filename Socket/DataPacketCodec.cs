...
namespace DasApp.Socket
{
    public class DataPacketCodec
    {
        ...
        public static string Encode(string encryptString, string encryptKey)
        {
            ...
            var cStream = new CryptoStream(mStream, dCSP.CreateEncryptor(rgbKey, new byte[8]),
        ...
        public static string Decode(string decryptString, string decryptKey)
        {
            ...
            var DCSP = new DESCryptoServiceProvider();
            var mStream = new MemoryStream();
            var cStream = new CryptoStream(mStream, DCSP.CreateDecryptor(rgbKey, new byte[8]),
                CryptoStreamMode.Write);
            cStream.Write(inputByteArray, 0, inputByteArray.Length);
            cStream.FlushFinalBlock();
